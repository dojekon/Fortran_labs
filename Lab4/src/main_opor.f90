...
use Environment
    ...
    allocate (X(Nx), Y(Ny), F(Nx*Ny)) ! Выделяем память под массивы
   ...
   close (Out)
    ...
    pure subroutine TabFuncImp(x1, y1, dx, dy, X, Y, F)
      ...
      ! Формирование вектора всех возможныъ значений переменной Х
      do concurrent (i = 1:Nx) ! Параллельные вычисления: данный тип цикла
         X(i) = x1 + dx*(i-1)   ! подразмевает независимость итераций друг от друга
      end do

      ! Формирование вектора всех возможныъ значений переменной Х
            do concurrent (j = 1:Ny) ! Параллельные вычисления: данный тип цикла
         Y(j) = y1 + dy*(j-1)   ! подразмевает независимость итераций друг от друга
      end do
   ...
   pure subroutine TabF(x1, y1, dx, dy, X, Y, F)
      ...
      F = [(((Sin(X(i)+Y)) / (((Cos(X(i))*Cos(Y))**2)-1)), i = 1, Nx)]
   end subroutine TabF
